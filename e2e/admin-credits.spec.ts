...
// ─── recharge: already wired in CreditUsersView, verify POST path ────

test.describe("credits recharge endpoint", () => {
  test("recharge button opens modal; submit POSTs to /v1/admin/credits/users/:id/recharge", async ({
    page,
  }) => {
    // Mock list so the page renders a row regardless of seed state
    await page.route("**/v1/admin/credits/users**", async (route: Route) => {
      const url = route.request().url();
      const method = route.request().method();

      // List endpoint: shape is { total, items: [{ account, user_id, username, nickname }] }
      if (method === "GET" && pathEndsWith(url, "/v1/admin/credits/users")) {
        await route.fulfill({
          status: 200,
          contentType: "application/json",
          body: ok({
      ...
      // Recharge endpoint (POST /credits/users/:id/recharge)
      if (
        method === "POST" &&
        pathMatches(url, /\/v1\/admin\/credits\/users\/\d+\/recharge$/)
      ) {
        await route.fulfill({
          status: 200,
          contentType: "application/json",
    ...
    const row = page.locator("table tbody tr").first();
    await expect(row).toBeVisible({ timeout: 10_000 });
    // CreditUsersView renders nickname || username. Our fixture has both set.
    await expect(row).toContainText("QA User");

    // Click "充值" button within the row (no data-test, use text)
    await row.getByRole("button", { name: /充值/ }).click();
    const modal = page.locator(".modal-overlay");
    await expect(modal).toBeVisible();
    await expect(modal.locator(".modal-title")).toContainText("充值额度");

    // Submit with the default form values
    const [rechargeRequest] = await Promise.all([
      page.waitForRequest(
        (req) =>
          req.method() === "POST" &&
          /\/v1\/admin\/credits\/users\/\d+\/recharge$/.test(
            new URL(req.url()).pathname,
          ),
        { timeout: 5_000 },
      ),
      modal.getByRole("button", { name: /确认充值/ }).click(),
    ]);

    // Payload shape sanity check: total_credits is a number
    const body = rechargeRequest.postDataJSON() as { total_credits: number };
