...
<template>
  <div class="page-container">
    <div class="page-header">
      <p class="page-breadcrumb">Billing / Orders</p>
    ...
    >
      ...
      <template #cell-user_id="{ row }">
        <span class="text-mono">{{ (row as Order).user_id }}</span>
      </template>

      <template #cell-payer_id="{ row }">
        <span class="text-mono">{{ (row as Order).payer_id }}</span>
      </template>
      ...
      <template #cell-amount="{ row }">
        <strong class="text-mono"
          >¥{{ centsToYuan((row as Order).amount) }}</strong
        >
      </template>
      ...
      <template #cell-pay_status="{ row }">
        <StatusBadge
          :status="String((row as Order).pay_status)"
          :map="payStatusMap"
        />
      </template>
      ...
      <template #cell-created_at="{ row }">
        <span class="text-muted">{{
          formatDate(String((row as Order).created_at))
        }}</span>
      </template>
    </DataTable>
  </div>
