...
import {
  HTTP_CHILD_ACCOUNT_FORBIDDEN,
  errorMessage,
  errorStatus,
} from "@/constants/agentErrno";
...
async function executeDelete() {
  ...
  try {
    ...
    toast.error(errorMessage(e, "下架失败"));
  } finally {
  ...
  <div class="agent-list">
    ...
    <div class="agent-list__header">
      <AppInput
        v-model="searchTerm"
        placeholder="搜索助手"
        class="agent-list__search"
      />
      <div class="agent-list__actions">
        <AppButton variant="primary" @click="router.push('/agents/new')">
          + 创建 Agent
        </AppButton>
        <AppButton
          variant="secondary"
          @click="router.push('/agents/new/from-template')"
        >
          从模板库选
        </AppButton>
      </div>
    ...
    >
      ...
      <template #cell-name="{ row }">
        <span class="agent-name">
          {{ row.name }}
          <span
            v-if="row.advanced_mode"
            class="badge badge--advanced"
            title="高级模式"
            >🔧</span
          >
      ...
      <template #cell-actions="{ row }">
        <div class="agent-list__row-actions">
          ...
          <AppButton size="sm" variant="secondary" @click="goDetail(row.id)">
            详情
          </AppButton>
          <AppButton size="sm" variant="secondary" @click="derive(row)">
            派生
          </AppButton>
