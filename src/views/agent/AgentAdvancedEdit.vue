...
import type { ToolFlags } from "@/types/agent";
...
const bodyDisplay = computed(() => {
  ...
  return (
    agent.value.custom_skill_body || agent.value.generated_skill_body || ""
  );
});
...
<template>
  <div class="advanced-editor">
    ...
    <!-- Loading state -->
    <div v-if="store.currentLoading" class="advanced-editor__loading">
      加载中…
    </div>
    ...
    <div v-else-if="store.currentError" class="advanced-editor__error">
      ...
      <AppButton variant="secondary" size="sm" @click="store.fetchOne(agentId)">
        重试
      </AppButton>
    </div>

    <!-- Not found state -->
    <div v-else-if="!agent" class="advanced-editor__empty">
      未找到对应 Agent。
    </div>
    ...
    <template v-else>
      <header class="advanced-editor__header">
        ...
        <span
          class="advanced-editor__char-count"
          :class="{ 'char-count--warn': charCount > 8000 }"
        >
          {{ charCount }} / 建议 ≤ 8000
        </span>
      </header>
      ...
      <section class="tool-flags">
        ...
        <label class="tool-flags__item tool-flags__item--dangerous">
          <input
            type="checkbox"
            v-model="toolFlags.dangerous"
            @change="onDangerousChange"
          />
          高危工具（谨慎开启）
        </label>
      </section>
      ...
      <footer class="advanced-editor__footer">
        <AppButton @click="handleSave" :loading="store.saving"
          >保存工具开关</AppButton
        >
    ...
    <ConfirmModal
      :visible="dangerousConfirmVisible"
      title="开启高危工具"
      :danger="true"
      confirmText="确认开启"
      cancelText="取消"
