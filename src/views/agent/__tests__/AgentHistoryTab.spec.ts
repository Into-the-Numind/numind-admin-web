...
import AgentHistoryTab from "@/views/agent/components/AgentHistoryTab.vue";
...
describe("AgentHistoryTab.vue", () => {
  beforeEach(() => {
    ...
    (agentApi.restoreAgentApi as ReturnType<typeof vi.fn>).mockResolvedValue(
      makeAgent({ version: 4 }),
    );
    (
      agentApi.listAgentHistoryApi as ReturnType<typeof vi.fn>
  ...
  // ----------------------------------------------------------------
  it("calls listAgentHistoryApi with the correct agentId on mount", async () => {
    await mountTab();
    expect(agentApi.listAgentHistoryApi).toHaveBeenCalledWith(AGENT_ID);
  });
  ...
  it("passes loading=true to DataTable while API is pending", async () => {
    (agentApi.listAgentHistoryApi as ReturnType<typeof vi.fn>).mockReturnValue(
  ...
  it("calls restoreAgentApi and shows success toast on confirm", async () => {
    ...
    expect(agentApi.restoreAgentApi).toHaveBeenCalledWith(
      AGENT_ID,
      expect.any(Number),
    );
