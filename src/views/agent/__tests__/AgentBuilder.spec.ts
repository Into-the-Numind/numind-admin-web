...
import AgentBuilder from "@/views/agent/AgentBuilder.vue";
import * as agentApi from "@/api/agent";
import type { Agent, SkillTemplate } from "@/types/agent";
...
const router = createRouter({
  ...
  routes: [
    ...
    { path: "/agents/builder", component: { template: "<div/>" } },
...
async function mountBuilder(opts: MountOptions = {}) {
  ...
  if (opts.query) {
    await router.push({ path: "/agents/builder", query: opts.query });
  } else {
    await router.push("/");
  }
...
describe("AgentBuilder — create mode (scratch)", () => {
  ...
  it("shows validation errors when save clicked with empty form", async () => {
    ...
    await flushPromises();
    // Should NOT call createAgentApi
    expect(agentApi.createAgentApi).not.toHaveBeenCalled();
  ...
  it("calls store.create and opens AfterSaveModal on valid form save", async () => {
    ...
    vi.mocked(agentApi.createAgentApi).mockResolvedValue(saved);
    ...
    expect(agentApi.createAgentApi).toHaveBeenCalled();
...
describe("AgentBuilder — create mode (from template)", () => {
  ...
  it("calls fetchTemplates and prefills form from template", async () => {
    ...
    vi.mocked(agentApi.listSkillTemplatesApi).mockResolvedValue([template]);

    const wrapper = await mountBuilder({
      query: { from: "template:1" },
    });

    expect(agentApi.listSkillTemplatesApi).toHaveBeenCalled();
    // Form should be prefilled with template name
    const form = wrapper.findComponent({ name: "QuestionnaireForm" });
    expect((form.props("modelValue") as { name: string }).name).toBe(
      "模板助手",
    );
  });

  it("passes source_template_id to createAgentApi when saving from template", async () => {
    ...
    const callArg = vi.mocked(agentApi.createAgentApi).mock.calls[0]?.[0];
...
describe("AgentBuilder — create mode (from copy)", () => {
  ...
  it("calls fetchOne(sourceId) and prefills name with '- 副本' suffix", async () => {
    ...
    vi.mocked(agentApi.getAgentApi).mockResolvedValue(source);

    const wrapper = await mountBuilder({ query: { from: "copy:5" } });

    expect(agentApi.getAgentApi).toHaveBeenCalledWith(5);
    const form = wrapper.findComponent({ name: "QuestionnaireForm" });
    expect((form.props("modelValue") as { name: string }).name).toBe(
      "原始助手 - 副本",
    );
...
describe("AgentBuilder — edit mode", () => {
  ...
  it("renders '编辑：...' title and fetches agent", async () => {
    ...
    vi.mocked(agentApi.getAgentApi).mockResolvedValue(agent);

    const wrapper = await mountBuilder({
      props: { mode: "edit", agentId: 10 },
    });

    expect(agentApi.getAgentApi).toHaveBeenCalledWith(10);
    expect(wrapper.text()).toContain("编辑：被编辑助手");
  });

  it("calls patchAgentApi (via store.update) on save in edit mode", async () => {
    const agent = makeAgent({ id: 10, name: "被编辑助手" });
    const updated = makeAgent({ id: 10, name: "被编辑助手" });
    vi.mocked(agentApi.getAgentApi).mockResolvedValue(agent);
    vi.mocked(agentApi.patchAgentApi).mockResolvedValue(updated);
    ...
    expect(agentApi.patchAgentApi).toHaveBeenCalledWith(10, expect.any(Object));
  });
...
describe("AgentBuilder — AfterSaveModal interactions", () => {
  ...
  it("navigates to agent detail on 'skip'", async () => {
    ...
    expect(router.currentRoute.value.path).toBe("/agents/77");
  });
  ...
  it("shows toast on 'trial-chat' then navigates to detail", async () => {
    ...
    expect(router.currentRoute.value.path).toBe("/agents/88");
  });
});
