...
describe("AgentList.vue", () => {
  beforeEach(() => {
    ...
    // Default: listAgentsApi returns empty list
    (agentApi.listAgentsApi as ReturnType<typeof vi.fn>).mockResolvedValue(
      emptyListResp,
    );
    (agentApi.deleteAgentApi as ReturnType<typeof vi.fn>).mockResolvedValue(
      undefined,
    );
  });
  ...
  it("shows empty state text when list is empty and not loading", async () => {
    ...
    expect(wrapper.text()).toContain("暂无助手，点击 + 创建第一个");
  });
  ...
  it("passes loading=true to DataTable while API is pending", async () => {
    // Make listAgentsApi hang indefinitely so loading stays true
    (agentApi.listAgentsApi as ReturnType<typeof vi.fn>).mockReturnValue(
      new Promise(() => {}),
    );
  ...
  it("shows error banner + retry when API rejects with 500", async () => {
    ...
    (agentApi.listAgentsApi as ReturnType<typeof vi.fn>).mockRejectedValue(err);
  ...
  it("opens ConfirmModal with danger when 下架 is clicked", async () => {
    ...
    // ConfirmModal should be hidden initially (visible=false)
    const modal = wrapper.findComponent(ConfirmModal);
    expect(modal.props("visible")).toBe(false);

    // Click the 下架 button in the action column
    const dangerBtns = wrapper
      .findAll("button")
      .filter((b) => b.text().includes("下架"));
    expect(dangerBtns.length).toBeGreaterThan(0);
    await dangerBtns[0].trigger("click");
    await wrapper.vm.$nextTick();

    // ConfirmModal should now be visible with danger=true and agent name in title
  ...
  // ----------------------------------------------------------------
  // 9. Confirming 下架 calls deleteAgentApi and shows success toast
  // ----------------------------------------------------------------
  it("calls deleteAgentApi with correct id when confirm is clicked", async () => {
    (agentApi.listAgentsApi as ReturnType<typeof vi.fn>).mockResolvedValue({
    ...
    expect(agentApi.deleteAgentApi).toHaveBeenCalledWith(42);
    expect(toastSpy.success).toHaveBeenCalledWith("已下架");
  });

  // ----------------------------------------------------------------
  // 10. Retry button re-invokes listAgentsApi
  // ----------------------------------------------------------------
  it("retry button calls listAgentsApi again after an error", async () => {
    const err = Object.assign(new Error("timeout"), {
      response: { status: 503, data: { message: "服务不可用" } },
    });
    (agentApi.listAgentsApi as ReturnType<typeof vi.fn>)
    ...
    expect(agentApi.listAgentsApi).toHaveBeenCalledTimes(2);
  });
});
