...
const maxVersion = computed(() =>
  store.history.reduce((m, h) => Math.max(m, h.version), 0),
);
...
<template>
  <div class="agent-history-tab">
      ...
      >
        ...
        <template #cell-actions="{ row }">
          <div class="actions-cell">
            ...
            <span
              v-if="(row as AgentHistory).version === currentVersion"
              class="current-badge"
              >当前版本</span
            >
            ...
            <template v-else>
              <AppButton
                variant="ghost"
                size="sm"
                @click="openView(row as AgentHistory)"
                >查看</AppButton
              >
              <AppButton
                variant="danger"
    ...
    <ConfirmModal
      ...
      :title="
        restoreTarget ? `确认恢复到 v${restoreTarget.version}？` : '确认恢复'
      "
      :message="
        restoreTarget
