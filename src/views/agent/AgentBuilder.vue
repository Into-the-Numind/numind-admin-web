...
import { initialFormState } from "@/types/agent";
import type {
  AgentFormState,
  CreateAgentPayload,
  PatchAgentPayload,
...
const isDirty = computed(
  () => JSON.stringify(form) !== initialFormSnapshot.value,
);
...
function formToPayload(): CreateAgentPayload {
  return {
    ...
    tool_flags:
      Object.keys(form.tool_flags).length > 0
        ? { ...form.tool_flags }
        : undefined,
...
async function confirmAdvancedToggle() {
  ...
  try {
    ...
    router.push(`/agents/${props.agentId}/edit`);
  } catch (e) {
...
<template>
  <div class="agent-builder">
    ...
    <header class="agent-builder__header">
      <h1 class="agent-builder__title">
        <template v-if="mode === 'edit'"
          >编辑：{{ form.name || "..." }}</template
        >
        <template v-else>创建新助手</template>
      </h1>
      <AppButton variant="primary" :loading="store.saving" @click="handleSave">
        保存
      </AppButton>
    </header>
    ...
    <div
      v-else-if="initError"
      class="agent-builder__feedback agent-builder__feedback--error"
    >
      <p>{{ initError }}</p>
    ...
    <ConfirmModal
      :visible="advancedConfirmVisible"
      title="切换到高级模式"
      confirm-text="确认切换"
      cancel-text="取消"
      :danger="true"
    ...
    >
      <p>切换到高级模式后，你可以直接编辑 Prompt 代码，但操作不可逆。</p>
      <p>确定要继续吗？</p>
    </ConfirmModal>
