...
async function fetchLogs() {
  ...
  try {
    const offset = (page.value - 1) * pageSize;
    const res = await getTierChangeLogsApi({
      from: from.value,
      to: to.value,
      offset,
      limit: pageSize,
...
<template>
  <div class="page-container">
    ...
    <div class="page-header">
      <div>
        <p class="page-breadcrumb">Users / Tier Changes</p>
        <h1 class="page-title">客户升级记录</h1>
        <p class="page-subtitle">
          查看直客 (B2) 对终端用户 (B3) 的等级变更记录，用于月度营收核算
        </p>
      </div>
      <div class="header-actions">
        <AppInput v-model="from" type="date" size="sm" />
        <span class="date-separator">&mdash;</span>
        <AppInput v-model="to" type="date" size="sm" />
        <AppButton
          variant="primary"
          size="sm"
          :loading="loading"
      ...
      <div v-if="stats" class="stats-grid">
        ...
        <div class="card card-body">
          ...
          <div class="tier-breakdown">
            <div
              v-for="b in stats.tier_breakdown"
              :key="b.new_tier"
              class="tier-row"
            >
              <span class="tier-name">{{
                tierBreakdownLabel(b.new_tier)
              }}</span>
              <span class="tier-count"
                >{{ b.count }}次 / {{ b.total_months }}月</span
              >
            </div>
            <div v-if="stats.tier_breakdown.length === 0" class="tier-empty">
              暂无数据
            </div>
          </div>
        </div>
      ...
      >
        <template #cell-created_at="{ row }">
          <span class="cell-text">{{
            formatDateTime((row as TierChangeLogItem).created_at)
          }}</span>
        </template>

        <template #cell-old_tier="{ row }">
          <StatusBadge
            :status="(row as TierChangeLogItem).old_tier"
            :map="tierLabels"
          />
        </template>
        ...
        <template #cell-new_tier="{ row }">
          <div class="tier-cell">
            <StatusBadge
              :status="(row as TierChangeLogItem).new_tier"
              :map="tierLabels"
            />
            <span
              v-if="
                isUpgrade(
                  (row as TierChangeLogItem).old_tier,
                  (row as TierChangeLogItem).new_tier,
                )
              "
              class="arrow-up"
              >&uarr;</span
        ...
        <template #cell-months="{ row }">
          <span class="cell-text"
            >{{ (row as TierChangeLogItem).months }}个月</span
          >
        </template>

        <template #cell-new_tier_expires="{ row }">
          <span class="cell-text">{{
            formatDateTime((row as TierChangeLogItem).new_tier_expires)
          }}</span>
        </template>
      </DataTable>
    </div>
