...
watch(
  [serviceTypeFilter, providerFilter, operationFilter, dateFrom, dateTo],
  () => {
    page.value = 1;
    fetchRecords();
  },
...
<template>
  <div class="page-container">
    <div class="page-header">
      <p class="page-breadcrumb">Billing / Records</p>
    ...
    >
      <template #cell-service_type="{ row }">
        <span class="label-badge">{{
          serviceTypeLabels[(row as UsageRecord).service_type] ||
          (row as UsageRecord).service_type
        }}</span>
      </template>

      <template #cell-provider="{ row }">
        <span>{{
          providerLabels[(row as UsageRecord).provider] ||
          (row as UsageRecord).provider
        }}</span>
      </template>

      <template #cell-operation="{ row }">
        <span>{{
          operationLabels[(row as UsageRecord).operation] ||
          (row as UsageRecord).operation
        }}</span>
      </template>

      <template #cell-total_tokens="{ row }">
        <span class="text-mono">{{
          (row as UsageRecord).total_tokens.toLocaleString()
        }}</span>
      </template>

      <template #cell-cost_cents="{ row }">
        <span class="text-mono">{{
          formatCost((row as UsageRecord).cost_cents)
        }}</span>
      </template>

      <template #cell-revenue_cents="{ row }">
        <span class="text-mono text-revenue">{{
          formatCost((row as UsageRecord).revenue_cents)
        }}</span>
      </template>

      <template #cell-created_at="{ row }">
        <span class="text-muted">{{
          formatDateTime(String((row as UsageRecord).created_at))
        }}</span>
      </template>
    </DataTable>
  </div>
