  ...
  <div class="page-container">
    ...
    >
      <template #cell-user_tier="{ row }">
        <StatusBadge
          :status="String((row as User).user_tier)"
          :map="tierLabels"
        />
      </template>
      ...
      <template #cell-tier_expires="{ row }">
        <span class="text-muted">
          {{
            (row as User).user_tier === "free"
              ? "永久"
              : (row as User).tier_expires
                ? formatDate(String((row as User).tier_expires))
                : "-"
          }}
        </span>
      </template>
      ...
      <template #cell-actions="{ row }">
        <div class="action-buttons">
          <AppButton
            ...
            variant="ghost"
            :disabled="processing"
            @click.stop="openTierModal(row as User)"
          >
            <Crown :size="14" />
            等级
          </AppButton>
        ...
        >
          <div class="modal-card" role="dialog" aria-modal="true">
            ...
            <div class="modal-actions">
              <AppButton variant="secondary" @click="tierModalVisible = false"
                >取消</AppButton
              >
              <AppButton
                variant="primary"
                :loading="processing"
                @click="submitTierChange"
                >确认变更</AppButton
              >
            </div>
          </div>
