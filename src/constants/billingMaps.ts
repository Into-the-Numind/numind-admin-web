...
// 供应商中文标签（与 llm_provider.name 完全对齐）
export const providerLabels: Record<string, string> = {
  "ali-dashscope": "阿里 DashScope",
  "volc-ark": "火山方舟",
  dmxapi: "DMXAPI",
  "dmxapi-ssvip": "DMXAPI SSVIP",
  aihubmix: "AiHubMix",
  "baidu-ocr": "百度 OCR",
  "bailian-file": "百炼文件服务",
  "funasr-local": "本地语音识别",
...
export const moduleGroups: Record<
  string,
  { label: string; operations: string[] }
> = {
  sop: {
    label: "SOP 工作流",
    operations: [
  ...
  salesrag: {
    label: "销售智能体",
    operations: [
      "salesrag_generate_answer",
      "salesrag_chat_generate",
      "salesrag_intent_analysis",
      "salesrag_strategy_select",
      "salesrag_rerank",
      "salesrag_tagging",
      "salesrag_ingest",
      "salesrag_ingest_upload",
      "salesrag_analyze_profile",
      "salesrag_analyze_profile_text",
      "salesrag_chat_style_text",
      "salesrag_chat_style_image",
      "salesrag_ocr",
    ],
  },
  other: {
...
export const serviceTypeFormOptions = Object.entries(serviceTypeLabels).map(
